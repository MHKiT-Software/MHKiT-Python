...
def get_variable_points(data, variable,x,y,z, time_step=-1):
    ...
    points= np.array([ [x, y, z] for x, y, z in zip(x_new, y_new, z_new)])
...
def get_all_data_points(data, variable,time_step):
    ...
    Returns
    -------
    known_points : float array 
        all grid point generated by d3d 
    v_all : float array 
        value to kown_points 
   ...
   # Layer_percentages= np.ma.getdata(data.variables['LayCoord_w'][:], False) # turbulent data 
    ...
    for i in range(len(Layer_percentages)):
        ...
        known_points = np.array([ [x, y, z] for x, y, z in zip(x_all, y_all, z_all)])
    return known_points, v_all 
...
def plot_data(data, variable, x=None , y=None, z=None, points=None , time_step = -1):
    ...
    '''
    known_points, v_all = get_all_data_points(data, var, time_step)

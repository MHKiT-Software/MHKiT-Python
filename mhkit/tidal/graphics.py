...
def _histogram_table(directions, velocities, width_dir, width_vel):
    ...
    -------
    ax: figure
        Text table of histogram data 
     '''

    # Get histgram and bin info
    H, dir_edges, vel_edges = _histogram(directions, velocities, width_dir, width_vel)
    #  Column and rows names for df
    rows    = [ f'{i:.1f}-{j:.1f}' for i,j in zip(dir_edges[:-1],dir_edges[1:])]
    columns = [ f'{i:.1f}-{j:.1f}' for i,j in zip(vel_edges[:-1],vel_edges[1:])]
    # Create DF
    table = pd.DataFrame.from_records(H, columns=columns, index = rows)  
    ...
    widths=[1.0/(1.5*len(columns))]*len(columns)
    # Plot the Table
    ax.table(cellText=table.values, colLabels=columns,rowLabels=rows, 
             loc='center', colWidths=widths, fontsize=14)
    fig.tight_layout()
    #TODO: Add x label and y label
    return ax

def _plot_histogram(directions, velocities, width_dir, width_vel):
    '''
    Returns the histogram joint probability as percent with directional bin rows 
    and velocity bin columns
    ...
    -------
    ax: figure
        Histogram cartesian with frquency contour 
 '''
    ...
    N_vel = int(vel_max/ width_vel)
    plt.figure()
    #plt.hexbin(directions, velocities, gridsize=(N_dir, N_vel))
    plt.hist2d(directions, velocities, bins=(N_dir, N_vel))

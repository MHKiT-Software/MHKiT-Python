...
def convert_time (data, time_index=None, time_stamp=None):
    '''
    Ouput the time_stamp for a time_index and vice versa. The optput catains 
    ...
    QoI: int, float
        ...
        the time_stamp. The time_index is a positive integer starting form 0 
        and incrementing until in simulation is complete. The time_stamp is the 
        seconds corresponding to those increments.
 
    '''
    assert type(data)== netCDF4._netCDF4.Dataset, 'data must be netCDF4 object'
    assert time_index or time_stamp, 'input of time_index or time_stamp needed'
    assert not(time_index and time_stamp), (f'only one time_index or time_stamp'
                                            +' can be input')
    assert isinstance(time_index, (int, float)) or isinstance(time_stamp, 

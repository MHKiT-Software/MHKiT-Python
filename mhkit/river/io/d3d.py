...
def get_all_time(data):
    '''
    Returns all of the data time stamps in seconds.
...
def seconds_to_index(data, seconds_run):
    '''
    The function will return a 'time_index' if passed a 'seconds_run'
...
def _convert_time(data, time_index=None, seconds_run=None):
    '''
    Converts an index to seconds or seconds to a index. The user 
    must specify 'time_index' or 'seconds_run' (Not both). The function 
    will returns 'seconds_run' if passed a 'time_index' or will return the closest
    'time_index' if passed a number of 'seconds_run'.
    ...
    QoI: int, float
        ...
        seconds corresponding to the 'time_index' increments.
    '''
...
def get_layer_data(data, variable, layer_index=-1 , time_index=-1):
    '''
    Get variable data from NetCDF4 object at a specified layer and timestep. If 
    ...
    layer_index: int
         A positve integer to pull out a layer from the dataset. 0 being closest 
         to the surface. Default is the bottom layer -1.
    time_index: int 
...
def get_all_data_points(data, variable, time_index= -1):  
    ...
    the Delft3D NetCDF4 object by iterating get_layer_data function. 
    ...
    variable: string
        ...
        found using "data.variables.keys()" in the console. 
    time_index: int
        A positive integer to pull the time step from the dataset. 
        Default is late time step -1.

...
def get_all_time(data):
    '''
    Returns all of the time stamps and their associated time index
    Parameters
    ----------
    data: NetCDF4 object 
    ...
    -------
     seconds_run: array
         Returns an array of integers that indicates the number of seconds after
        the simulation start that the NetCDF4 data in the data object was recorded.
    '''
...
def convert_time(data, time_index=None, seconds_run=None):
    '''
    Returns 'seconds_run' if passed a 'time_index' or will return the closest
    'time_index' if passed a number of 'seconds_run'. The user must only 
    specify 'time_index' or 'seconds_run'.
    ...
    seconds_run: int, float
        ...
        that the simulation has been running

    Returns
    -------
    QoI: int, float
        The quantity of interest is the unknown value either the time_index or
        the seconds_run. The time_index is a positive integer starting from 0 
...
def get_layer_data(data, variable, layer_index= -1 , time_index=-1):
    '''
    Get variable data from NetCDF4 object at specified layer and timestep. If 
    the array is 2D with no layers the layer_index will not change the output. 
...
def get_all_data_points(data, variable, time_index= -1):  
    '''
    Get data points from all layers in NetCDF4 object generated from Delft3D using 
...
def turbulent_intensity(data, points='cells', time_index= -1,
    ...
    specified points. Assumes variable names: turkin1, ucx, ucy and ucz.
    ...
    data: NetCDF4 object 
       ...
       stress, generated by running a Delft3D model.  
    points: string, DataFrame  
        Point to interpolate data onto. 
          'cells': interpolates all data into velocity coordinate system (Default)
          'faces': interpolates all data into TKE coordinate system 
          DataFrame of x, y, and z coordinates: Interpolates data onto user 
          provided points 
    time_index: int 
    ...
    intermediate_values: boolean (optional)
        If true will return position(x,y,z) and values need to calculate
        turbulent intensity (ucx, uxy, uxz and turkin1) in a Dataframe. If false
        will only return position and turbulent intensity values. False 

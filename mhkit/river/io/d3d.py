...
def get_all_timestamps (data):
    '''
    retuns all of the time stamps and their associated time index
    Parameters
    ----------
    data : netcdf4 object 
    ...
    -------
    times : DataFrame
        DataFrame with colume names 'time_index' and 'time_stamps'
        The 'time_index' is an int that correlateds to a 'times_stamp' for the
        amount of seconds the simulation has been running  

    '''
    assert type(data)== netCDF4._netCDF4.Dataset, 'data must be netCDF4 object'
        
    max_time= data['time'].shape
    max_time= int(''.join(map(str, max_time)))
    time_indexes= np.linspace(0, max_time-1, num=max_time)
    ...
    times = np.array([ [indexes, stamps] for indexes, stamps, in zip(time_indexes, time_stamps)])
    times= pd.DataFrame(times, columns=['time_index','time_stamp'])
    
    return times

def convert_time (data, time_index = False, time_stamp= False):
    '''
    Ouput the time_stamp for a time_index and vis versa. The optput catains 
    both the time_index and time_stamp. 
    ...
    if time_index:
        index = times[times.eq(time_index).any(1)].index
        time_stamp = times.iloc[index[0]]['time_stamp']
        
    elif time_stamp:
        index = times[times.eq(time_stamp).any(1)].index
        time_index = times.iloc[index[0]]['time_index']
    else:
        print('no valid time_step or time_index input')
    
    return time_index, time_stamp
...
def get_layer_data(data, variable, layer_index= -1 , time_index=-1):
    ...
    max_time= data['time'].shape
    max_time_index= int(''.join(map(str, max_time)))-1 # to account for zero index
    ...
    time= np.ma.getdata(data.variables['time'][i], False)*np.ones(len(x))

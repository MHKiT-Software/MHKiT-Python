...
def get_layer_data(data, variable, layer_index= -1 , time_index=-1):
    ...
    ----------
    data : netcdf4 object 
       A netCDF object (network common data form) that contains spatial data 
       such as velocity of shear stess within a river. These can be generated by 
...
def variable_interpolation(data, variables, points='cells'):
    ...
    '''
    assert isinstance(points, (str, pd.DataFrame)),'points must be a string or DataFrame'
...
def get_all_data_points(data, variable, time_index= -1):  
    '''
    Get data points from all layers in netcdf file generated from Delft3D using 
...
def unorm(x, y ,z):
    ...
    assert all([len(x) == len(y), len (y) ==len (z)]), 'lengths of arrays must mathch'
...
def turbulent_intensity(data, points='cells', time_index= -1, intermediate_values = False ):
    ...
    intermediate_values: boolean
        ...
        if false will only return x,y,z, and turbulent intesity values. False is the default value      
        
    Returns
    -------
    TI_data : Dataframe
        If intermediate_values is true all values are output 
        If intermediate_values is equal to false only turbulent_intesity and x, y, and z varibles are output 
            turbulet_intesity- turbulent kinetic energy divided by the root mean squared velocity
    ...
    zero_bool= np.isclose( TI_data['turkin1'][ TI_data['turkin1']<0].array, 
                          np.zeros(len( TI_data['turkin1'][ TI_data['turkin1']<0].array)),atol=1.0e-4)

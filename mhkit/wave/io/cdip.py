...
def _start_and_end_of_year(year):
    ...
    if not isinstance(year, (type(None), int, list)):
        raise ValueError(
            'year must be of type int, list, or None. Got: {type(year)}')
...
def request_netCDF(station_number, data_type):
    ...
    if not isinstance(data_type, str):
        raise ValueError(
            f'data_type must be of type string. Got: {type(data_type)}')
    ...
    if data_type == 'historic':
        data_url = f'{BASE_URL}archive/{station_number}p1/{station_number}p1_historic.nc'
...
def get_netcdf_variables(nc, start_date=None, end_date=None,
    ...
    if not isinstance(all_2D_variables, bool):
        raise ValueError(
            'all_2D_variables must be a boolean. Got: {all_2D_variables}')

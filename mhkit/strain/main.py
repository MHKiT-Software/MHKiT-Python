...
def calculate_loads(rosette1, rosette2, elastic_modulus, shear_modulus, shear_width, transverse_width, radius):
    normal = 0.5 * (rosette1['axial_strain_x'] + rosette2['axial_strain_x']) * \
             elastic_modulus * (shear_width * transverse_width - np.pi * radius**2)
    moment = (rosette1['axial_strain_x'] - rosette2['axial_strain_x'])/shear_width * \
             elastic_modulus * (transverse_width*shear_width**3 / 12 - np.pi*radius**4 / 4)
    torsion1 = calculate_torsion(rosette1['shear_strain'], shear_modulus, shear_width, transverse_width, radius)
    torsion2 = calculate_torsion(rosette2['shear_strain'], shear_modulus, shear_width, transverse_width, radius)

    return normal, moment, torsion1, torsion2

def calculate_torsion(shear_strain, shear_modulus, shear_width, transverse_width, radius):

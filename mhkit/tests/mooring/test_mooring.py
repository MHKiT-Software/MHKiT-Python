...
class TestMooring(unittest.TestCase):
    ...
    def test_laylength(self):
        fpath = join(datadir, 'line1_test.nc')
        ds = xr.open_dataset(fpath)
        laylengths = mooring.general.laylength(ds, depth=-56, tolerance=0.25)
        laylength = laylengths.mean().values
        self.assertAlmostEqual(laylength, 45.0, 1)

    def test_animate3d(self):
        fpath = join(datadir, 'line1_test.nc')
        ds = xr.open_dataset(fpath)
        dsani = ds.sel(Time=slice(0,10))
        ani = mooring.graphics.animate3d(dsani, interval=10, repeat=True, 
    ...
    def test_animate2d(self):
        ...
        ds = xr.open_dataset(fpath)
        dsani = ds.sel(Time=slice(0,10))
        ani2d = mooring.graphics.animate2d(dsani, xaxis='x',yaxis='z', repeat=True, 

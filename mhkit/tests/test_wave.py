...
class TestIOcdip(unittest.TestCase):
    ...
    def test_get_netcdf_variables(self):
        ...
        time_variables, metadata = wave.io.cdip.get_netcdf_variables(nc, 
            include_2D_variables=True)
        
        import ipdb; ipdb.set_trace()
        
    # def test_cdip_multiyear(self):
        # station_number = '067'
        # year1=2011
        # year2=2013
        # year = [year1, year2]
        # data, units = wave.io.cdip.request_data(station_number,years=year)
        # expected_index0 = datetime(year1,1,1)        
        # self.assertEqual(data.index[0].floor('d'), expected_index0)
        # expected_index_final = datetime(year2,12,31)
        # self.assertEqual(data.index[-1].floor('d'), expected_index_final) 

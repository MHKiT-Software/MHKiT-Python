...
class TestIOcdip(unittest.TestCase):
    ...
    # def test_get_netcdf_variables_params(self):
        ...
        # returned_keys_1D = [key for key in data['vars1D'].keys()]
        # returned_keys_2D = [key for key in data['vars2D'].keys()]
        # returned_keys_metadata = [key for key in data['metadata'].keys()]
    ...
    # def test_get_netcdf_variables_time_slice(self):
        ...
        # self.assertTrue(data['vars1D'].index[0] > start_dt)
        # self.assertTrue(data['vars1D'].index[-1] < end_dt)
    ...
    # def test_parse_data_multiyear(self):
        ...
        # vars1D = data['vars1D']
        # self.assertEqual(vars1D.index[0].floor('d'), expected_index0)
        # self.assertEqual(vars1D.index[-1].floor('d'), expected_index_final) 
        
        # for key,var2D  in data['vars2D'].items():
            # self.assertEqual(var2D.index[0].floor('d'), expected_index0)
            # self.assertEqual(var2D.index[-1].floor('d'), expected_index_final) 

...
class TestDevice(unittest.TestCase):
    ...
    def test_harmonics_sine_wave_xarray(self):
        current = xr.DataArray(data=self.signal, 
                               dims='index',
    ...
    def test_harmonic_subgroup_sine_wave_xarray(self):
        harmonics = xr.Dataset(data_vars={'harmonics':(['index'], self.harmonics_vals)},
    ...
    #     self.assertEqual(P.sum()['Gross'],
    #                       (voltage.values * current.values).sum())
    ...
    #     P1 = power.characteristics.ac_power_three_phase(
    #         voltage, current, 1, False)
    #     P1b = power.characteristics.ac_power_three_phase(
    #         voltage, current, 0.5, False)
    #     P2 = power.characteristics.ac_power_three_phase(
    #         voltage, current, 1, True)
    #     P2b = power.characteristics.ac_power_three_phase(
    #         voltage, current, 0.5, True)

    #     self.assertEqual(P1.sum().iloc[0], 584)
    #     self.assertEqual(P1b.sum().iloc[0], 584/2)
    #     self.assertAlmostEqual(P2.sum().iloc[0], 1011.518, 2)
    #     self.assertAlmostEqual(P2b.sum().iloc[0], 1011.518/2, 2)

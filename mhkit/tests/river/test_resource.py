...
class TestResource(unittest.TestCase):
    ...
    def test_velocity_to_power(self):
        ...
        self.assertAlmostEqual(
            (P['P'][1:-1] - 10*V['V'][1:-1]).sum(), 0.00, places=2)
    ...
    def test_plot_power_duration_curve(self):
        filename = abspath(
            join(plotdir, 'river_plot_power_duration_curve.png'))
        if isfile(filename):
            os.remove(filename)
    ...
    def test_plot_velocity_duration_curve(self):
        filename = abspath(
            join(plotdir, 'river_plot_velocity_duration_curve.png'))
        if isfile(filename):
            os.remove(filename)
    ...
    def test_plot_discharge_timeseries(self):
        filename = abspath(
            join(plotdir, 'river_plot_discharge_timeseries.png'))
        if isfile(filename):
            os.remove(filename)
    ...
    def test_plot_discharge_vs_velocity(self):
        filename = abspath(
            join(plotdir, 'river_plot_discharge_vs_velocity.png'))
        if isfile(filename):
            os.remove(filename)

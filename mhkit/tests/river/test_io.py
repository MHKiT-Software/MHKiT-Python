...
class TestIO(unittest.TestCase):
    ...
    def test_load_usgs_data_daily(self):
        ...
        self.assertEqual(data.columns, ['Discharge, cubic feet per second'])
        self.assertEqual(
            (data.index == expected_index.tz_localize('UTC')).all(), True)
    ...
    def test_convert_time(self):
        data = self.d3d_flume_data
        time_index = 2
        seconds_run = river.io.d3d.index_to_seconds(
            data, time_index=time_index)
        seconds_run_expected = 120
        self.assertEqual(seconds_run, seconds_run_expected)
        seconds_run = 60
        time_index = river.io.d3d.seconds_to_index(
            data, seconds_run=seconds_run)
    ...
    def test_layer_data(self):
        ...
        for var in variable:
            layer = 2
            time_index = 3
            layer_data = river.io.d3d.get_layer_data(
                data, var, layer, time_index)
            layer_compare = 2
            ...
            assert_array_almost_equal(
                layer_data.x, layer_data_expected.x, decimal=2)
            assert_array_almost_equal(
                layer_data.y, layer_data_expected.y, decimal=2)
            assert_array_almost_equal(
    ...
    def test_variable_interpolation(self):
        ...
        self.assertEqual(np.size(transformes_data['ucx']), np.size(
            transformes_data['turkin1']))
        transformes_data = river.io.d3d.variable_interpolation(
    ...
    def test_unorm(self):
        ...
        unorm_expected = [
            np.sqrt(1**2+1**2+1**2), np.sqrt(2**2+2**2+2**2), np.sqrt(3**2+3**2+3**2)]
    ...
    def test_turbulent_intensity(self):
        ...
        test_points = np.array([[x, y, waterdepth] for x, y, waterdepth in zip(
            x_test, y_test, waterdepth_test)])
        ...
        for var in TI_vars:
            # get all data
            var_data_df = river.io.d3d.get_all_data_points(
                data, var, time_index)
        ...
        u_mag = river.io.d3d.unorm(
            TI_data['ucx'], TI_data['ucy'], TI_data['ucz'])
